...
function CountConditionalSteps() {
  const [jsonInput, setJsonInput] = useState('');
  const [response, setResponse] = useState([]);
  const { isDarkMode, toggleTheme } = useContext(ThemeContext); // Pobieranie motywu z kontekstu

  const handleSubmit = async (event) => {
    event.preventDefault();
    try {
      const res = await fetch('/count-conditional-steps', {
        method: 'POST',
        headers: {
  ...
  return (
    ...
    >
      ...
      <h1 style={{ color: isDarkMode ? '#61dafb' : '#4a90e2' }}>Policz Kroki Warunkowe</h1>
      <form
...
export default CountConditionalSteps;
